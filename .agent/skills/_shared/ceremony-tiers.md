...
| **L** | New subsystem, new service, new project, architectural change, migration, new auth/payment/multi-tenancy *architecture* (not a routine feature built on existing auth patterns — see the escalation rule below) | Discovery → PRD → TRD → Architecture → ADR → UI/UX → task breakdown → approval gate → M-tier loop per task. Full document set required. |
...
2. **Escalate one tier if any of these is true:**
   - Introduces new authentication/authorization *architecture*, a new
     trust boundary, or multi-tenancy — not the same as using an
     existing framework's stock auth pattern for a routine feature
     (e.g. a standard registration+verification flow built on Laravel's
     built-in `MustVerifyEmail` is M; designing a new SSO/RBAC system is L)
   - Touches payments or PII
